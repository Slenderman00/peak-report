...
An example of this can be seen in \textit{Figure \ref{fig:JWTreq}} where the client requests a video claim from the video service,
so that it can prove ownership of a video to the video service.
\newpage
...
\subsection{Client-Mediated communication}
...
This allows for service to service communication that always scales 1:1 with the number of users. This helps 
reduce service-side complexity and allows for a more robust system. 
This approach greatly simplifies the service-to-service communication and is a radical departure from the current cloud computing paradigm.
However, this approach comes with some caveats, as the client is now responsible for the communication between services, Each
microservice must be designed to function without
