...
\subsubsection{Cluster-Level DDoS mitigation}
...
\begin{enumerate}
    ...
    \item \textbf{Address removal} - The pods affected pods disable their network interfaces.
    \item \textbf{DNS Resolution blocking} - PeakDNS blocks the IP addresses belonging to the attackers.
    \item \textbf{Address reacquisition} - Each pod re-enables their network interfaces while also requesting new IP addresses from the DHCP server.
    \item \textbf{Metrics-based record management} - PeakDNS updates the DNS records with the new IP addresses.
    \item \textbf{The attack is mitigated} - The attack is mitigated by blocking the malevolent IP addresses from reading the new DNS records.
\end{enumerate}
...
As one can see in \textit{Figure \ref{fig:service-dns-mapping}} the video services looks quite conventional.
